...
// resolveOpenAIUpstreamEndpoint 返回 OpenAI 兼容请求实际使用的上游端点。
// 转发结果优先于运行时上下文；两者都缺失时，才根据账号和入站端点回退推导。
//
// @param c Gin 请求上下文。
// @param account 实际参与转发的账号。
// @param result 转发结果，可为空。
// @return 用于 usage 和运维日志的实际上游端点。
