...
// ExtraKeyJSONSchemaToJSONObject 是 accounts.extra 中控制结构化输出兼容模式的键名。
// 值类型必须为 bool；仅 OpenAI APIKey 账号读取该配置。
...
//
// @param extra 账号的 accounts.extra 数据。
// @return 仅当目标键为 bool true 时返回 true。
