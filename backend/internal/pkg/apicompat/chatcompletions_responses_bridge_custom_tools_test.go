...
func TestChatCompletionsResponseToResponses_ExplicitFunctionOwnsCustomAliasCollision(t *testing.T) {
	resp := &ChatCompletionsResponse{ServiceTier: "priority", Choices: []ChatChoice{{Message: ChatMessage{ToolCalls: []ChatToolCall{{
	...
	events := ChatCompletionsResponseToResponsesEvents(resp, "deepseek-test",
		map[string]bool{"exec": true}, map[string]bool{"functions__exec": true}, false,
		map[string]NamespacedToolName{"functions__wait": {Namespace: "functions", Name: "wait"}})
	completed := events[len(events)-1]
	require.Equal(t, "response.completed", completed.Type)
	require.NotNil(t, completed.Response)
	assert.Equal(t, "priority", completed.Response.ServiceTier)
	...
	assert.Equal(t, `{"path":"/tmp"}`, completed.Response.Output[0].Arguments)
}
