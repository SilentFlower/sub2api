...
// DowngradeResponsesJSONSchemaToJSONObject 将 Responses 请求的输出格式降级为 JSON Object。
//
// 参数：
//   - body：Responses API 请求体。
//
// 返回值：转换后的请求体、是否发生转换，以及解析或序列化错误。
...
// DowngradeChatJSONSchemaToJSONObject 将 Chat Completions 请求的输出格式降级为 JSON Object。
//
// 参数：
//   - body：Chat Completions API 请求体。
//
// 返回值：转换后的请求体、是否发生转换，以及解析或序列化错误。
func DowngradeChatJSONSchemaToJSONObject(body []byte) ([]byte, bool, error) {
	...
	insertAt := leadingInstructionMessageCount(messages)
...
func leadingInstructionMessageCount(messages []json.RawMessage) int {
