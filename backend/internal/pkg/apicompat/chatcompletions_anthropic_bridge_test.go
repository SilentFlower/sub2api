...
func TestAnthropicToChatCompletionsRequest_BasicText(t *testing.T) {
	...
	require.JSONEq(t, `[{"type":"text","text":"hello"}]`, string(out.Messages[0].Content))
...
func TestAnthropicToChatCompletionsRequest_SystemPrompt(t *testing.T) {
	...
	require.JSONEq(t, `[{"type":"text","text":"You are helpful"}]`, string(out.Messages[0].Content))
}
...
func TestChatCompletionsResponseToAnthropic_SkipsToolCallWithoutName(t *testing.T) {
	resp := &ChatCompletionsResponse{
		ID:    "chatcmpl-missing-name",
		Model: "gpt-5.4",
		Choices: []ChatChoice{{
			Message: ChatMessage{
				Role: "assistant",
				ToolCalls: []ChatToolCall{{
					ID:       "call_anon",
					Type:     "function",
					Function: ChatFunctionCall{Arguments: `{"a":1}`},
				}},
			},
			FinishReason: "tool_calls",
		}},
	...
	out := ChatCompletionsResponseToAnthropic(resp, "gpt-5.4")

	require.Len(t, out.Content, 1)
	require.Equal(t, "text", out.Content[0].Type)
	require.Empty(t, out.Content[0].Text)
	require.Equal(t, "end_turn", AnthropicStopReasonString(out.StopReason))
}
...
// TestDirectBridge_RequestMatchesDoubleConversion 验证直连桥保留双转换链的字段
// 语义，同时对消息正文使用更稳定的 typed content part。
func TestDirectBridge_RequestMatchesDoubleConversion(t *testing.T) {
	...
	// 消息顺序和工具调用语义对齐，但非 tool 正文固定为 typed content part，
	// 不跟随双转换链在 string/array 之间切换。
	...
	for i := range direct.Messages {
		...
		if direct.Messages[i].Role != "tool" && len(direct.Messages[i].Content) > 0 {
			var parts []ChatContentPart
			require.NoError(t, json.Unmarshal(direct.Messages[i].Content, &parts), "msg %d content must use typed parts", i)
			require.NotEmpty(t, parts, "msg %d content parts must not be empty", i)
		}
...
func TestChatCompletionsToAnthropicStreamState_ToolCallNameNeverArrives(t *testing.T) {
	// function name 始终缺失时无法构造合法 tool_use，应跳过该调用并正常结束。
	events := collectAnthropicStreamEvents(t, []string{
...
func TestAnthropicToChatCompletionsRequest_UserArrayContentKeepsTypedParts(t *testing.T) {
	// 纯文本数组也保留 typed content part，避免相同历史因多模态与否改变 JSON 形态。
	req := &AnthropicRequest{
		Model:     "deepseek-v4-pro",
		MaxTokens: 100,
		Messages: []AnthropicMessage{
	...
	require.NoError(t, err)
	require.Len(t, out.Messages, 1)
	require.JSONEq(t, `[{"type":"text","text":"first"},{"type":"text","text":"second"}]`, string(out.Messages[0].Content))
}

func TestDirectBridge_ArrayUserContentKeepsStableShape(t *testing.T) {
	// 纯文本和含图片的用户消息都使用同一种数组形态，稳定 prefix cache 边界。
	req := &AnthropicRequest{
		Model:     "deepseek-v4-pro",
		MaxTokens: 100,
		Messages: []AnthropicMessage{
	...
	require.Len(t, direct.Messages, 3)
	for i := range direct.Messages {
		var parts []ChatContentPart
		require.NoError(t, json.Unmarshal(direct.Messages[i].Content, &parts), "msg %d content must use typed parts", i)
		require.NotEmpty(t, parts, "msg %d content parts must not be empty", i)
