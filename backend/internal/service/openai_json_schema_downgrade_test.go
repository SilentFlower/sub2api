...
func TestApplyOpenAIJSONSchemaDowngrade(t *testing.T) {
	enabled := &Account{
		...
		Extra:    map[string]any{openai_compat.ExtraKeyJSONSchemaToJSONObject: true},
	}
	...
	t.Run("invalid json keeps original validation path", func(t *testing.T) {
		body := []byte(`{"broken"`)
		out, err := applyOpenAIJSONSchemaDowngrade(nil, enabled, body, openAIJSONSchemaRequestShapeResponses, openAIResponsesEndpoint)
		require.NoError(t, err)
		require.Equal(t, body, out)
...
func TestResolveOpenAIJSONSchemaUpstreamEndpoint(t *testing.T) {
	require.Equal(t, openAIResponsesEndpoint, resolveOpenAIJSONSchemaUpstreamEndpoint(&Account{Type: AccountTypeOAuth}))
	require.Equal(t, openAIResponsesEndpoint, resolveOpenAIJSONSchemaUpstreamEndpoint(&Account{Type: AccountTypeAPIKey}))
	require.Equal(t, openAIChatCompletionsEndpoint, resolveOpenAIJSONSchemaUpstreamEndpoint(&Account{
		Type:  AccountTypeAPIKey,
