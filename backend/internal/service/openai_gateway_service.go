...
// OpenAIForwardResult 表示一次 OpenAI 兼容请求的转发结果，供响应转换、用量记录和计费使用。
type OpenAIForwardResult struct {
	...
	// （分组单价 × 次数 × 倍率）。
	WebSearchCalls int
	// SearchCount 是 Grok 原生 web_search 或工具搜索调用次数，用于按千次计费。
	SearchCount int
...
// GetActualOpenAIUpstreamEndpoint 返回当前请求最近一次转发尝试记录的实际上游端点。
//
// @param c Gin 请求上下文。
// @return 已记录的上游端点；未记录时返回空字符串。
