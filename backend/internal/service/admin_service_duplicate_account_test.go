...
func TestDuplicateAccountCopiesConfigurationAndResetsRuntimeState(t *testing.T) {
	...
	duplicateCredentials, ok := duplicate.Credentials["nested"].(map[string]any)
	require.True(t, ok)
	duplicateCredentials["token"] = "changed"
	duplicateConfig, ok := duplicate.Extra["config"].(map[string]any)
	require.True(t, ok)
	duplicateConfig["region"] = "changed"
	duplicateItems, ok := duplicate.Extra["items"].([]any)
	require.True(t, ok)
	require.NotEmpty(t, duplicateItems)
	duplicateItem, ok := duplicateItems[0].(map[string]any)
	require.True(t, ok)
	duplicateItem["enabled"] = false
	storedSource, getErr := repo.GetByID(ctx, source.ID)
	require.NoError(t, getErr)
	storedCredentials, ok := storedSource.Credentials["nested"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "source-token", storedCredentials["token"])
	storedConfig, ok := storedSource.Extra["config"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "us-east-1", storedConfig["region"])
	storedItems, ok := storedSource.Extra["items"].([]any)
	require.True(t, ok)
	require.NotEmpty(t, storedItems)
	storedItem, ok := storedItems[0].(map[string]any)
	require.True(t, ok)
	require.Equal(t, true, storedItem["enabled"])
