...
type rateLimitClearRepoStub struct {
	...
	clearAntigravityOverages    int
	clearModelRateLimitCalls    int
	clearTempUnschedCalls       int
	clearErrorErr               error
	clearRateLimitErr           error
	clearAntigravityErr         error
	clearAntigravityOveragesErr error
...
func (r *rateLimitClearRepoStub) ClearAntigravityOveragesState(ctx context.Context, id int64) error {
	r.clearAntigravityOverages++
	return r.clearAntigravityOveragesErr
}
...
func TestRateLimitService_ClearRateLimit_AlsoClearsTempUnschedulable(t *testing.T) {
	...
	require.Equal(t, 1, repo.clearAntigravityOverages)
...
func TestRateLimitService_ClearRateLimit_ClearRateLimitFailed(t *testing.T) {
	...
	require.Equal(t, 0, repo.clearAntigravityOverages)
	require.Equal(t, 0, repo.clearAntigravityCalls)
...
func TestRateLimitService_ClearRateLimit_ClearAntigravityOveragesFailed(t *testing.T) {
	repo := &rateLimitClearRepoStub{
		clearAntigravityOveragesErr: errors.New("clear antigravity overages failed"),
	}
	cache := &tempUnschedCacheRecorder{}
	svc := NewRateLimitService(repo, nil, &config.Config{}, nil, cache)

	err := svc.ClearRateLimit(context.Background(), 121)
	require.Error(t, err)
