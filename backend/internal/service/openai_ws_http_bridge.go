...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	if account.Platform != PlatformGrok && isOpenAIResponsesLiteWebSocketPayload(payload) {
		liteBody, liteChanged, liteErr := s.applyOpenAIResponsesLiteHTTPBodyPolicy(
			ctx, account, body, strings.TrimSpace(gjson.GetBytes(body, "model").String()), "true",
		)
		if liteErr != nil {
	...
	buildUpstreamRequest := func(requestBody []byte) (*http.Request, error) {
		...
		s.applyOpenAIResponsesLiteWSHTTPBridgePolicy(ctx, upstreamReq, account, payload, requestBody, originalModel)
	...
	resultWithUsage := func() *OpenAIForwardResult {
		...
		result := &OpenAIForwardResult{
			...
			ReasoningEffort:               ApplyThinkingEnabledFallback(extractOpenAIUpstreamReasoningEffort(body, originalModel, mappedModel), body, mappedModel),
