...
func TestOpenAIGatewayService_ResponsesUnknownModelDoesNotFallbackToGPT54(t *testing.T) {
	...
	originalBody := []byte(`{"model":"gpt-unknown-model","stream":false,"instructions":"local-test-instructions","input":[{"type":"text","text":"hi"}]}`)
	...
	require.Equal(t, "gpt-unknown-model", gjson.GetBytes(upstream.lastBody, "model").String())
...
func TestOpenAIGatewayService_OAuthPassthrough_GPT56ModelsUseCurrentCodexIdentity(t *testing.T) {
	...
	for _, model := range models {
		t.Run(model, func(t *testing.T) {
			...
			require.Contains(t, req.Header.Get("User-Agent"), codexCLIVersion)
		})
	}
}
...
func TestOpenAIGatewayService_APIKeyPassthrough_PreservesBodyAndUsesResponsesEndpoint(t *testing.T) {
	...
	originalBody := []byte(`{"model":"gpt-5.2","stream":false,"service_tier":"flex","max_output_tokens":128,"reasoning":{"effort":"max"},"input":[{"type":"text","text":"hi"}]}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"model_mapping": map[string]any{"gpt-5.2": "gpt-5.6-sol"},
		},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
