...
//  4. 身份候选：官方 UA / 官方 originator / 全局白名单 / 自定义 UA / App Server 开闸（全局开关 OR 账号开关）；都不命中 → 拒（NotMatchedUA）。
...
func (d *OpenAICodexClientRestrictionDetector) Detect(c *gin.Context, account *Account, policy CodexRestrictionPolicy, body []byte) CodexClientRestrictionDetectionResult {
	...
	// 4. 身份候选（优先级：官方 > 全局白名单 > 自定义 UA > App Server 开闸）。
	reason := ""
	...
	default:
		...
		} else if patterns := account.GetCodexCLIOnlyCustomUserAgentPrefixes(); len(patterns) > 0 &&
	...
	//    白名单/自定义 UA/App Server 候选可能不带可解析引擎版本，整块跳过。
