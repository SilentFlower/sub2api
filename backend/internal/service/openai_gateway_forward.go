...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if result, handled, webSearchErr := s.handleOpenAIResponsesWebSearch(ctx, c, account, body); handled {
		return result, webSearchErr
	}
	...
	// 在进入 passthrough、Codex transform 或原生 Chat Completions 分支前，
	// 先修复显式为 null 的工具 Schema type，避免同一无效请求在账号池中反复重试。
	...
	if passthroughEnabled {
		...
		// 透传分支只需要轻量提取字段，避免热路径全量 Unmarshal；模型候选必须与实际
		// Forward 路径一致，否则 API Key/compact 会按未发送的账号映射记录 effort。
		upstreamModel := resolveOpenAIAccountUpstreamModelForRequest(account, reqModel, compactPath)
		reasoningEffort := extractOpenAIUpstreamReasoningEffort(body, reqModel, upstreamModel)
	...
	if imageGenerationAllowed && !isCompactRequest && (codexImageGenerationBridgeEnabled || isOpenAIImageGenerationModel(requestView.Model) || openAIRequestBodyImageGenerationToolNeedsNormalization(body) || isOpenAIImageGenerationModel(upstreamModel)) {
		...
		if normalizeOpenAIResponsesImageOnlyModel(decoded, s.openAIImageGenerationMainModel(ctx)) {
			markDecodedModified()
