...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	resolvedUpstreamModel := resolveOpenAIAccountUpstreamModelForRequest(
		account,
		reqModel,
		isOpenAIResponsesCompactPath(c),
	)
	if isOpenAIResponsesCompactPath(c) {
		if fallbackModel := s.resolveOpenAICompactFallbackModel(account, reqModel); fallbackModel != "" {
			resolvedUpstreamModel = fallbackModel
		}
	}
	if resolvedUpstreamModel != "" && resolvedUpstreamModel != reqModel {
		nextBody, setErr := sjson.SetBytes(body, "model", resolvedUpstreamModel)
		if setErr != nil {
			return nil, fmt.Errorf("set passthrough upstream model: %w", setErr)
		}
		body = nextBody
		upstreamPassthroughModel = resolvedUpstreamModel
