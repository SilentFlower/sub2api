...
func (s *AccountTestService) testOpenAIImageOAuth(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	imageSettings := s.settingService.getOpenAIImageGenerationSettingsCached(ctx)
	responsesBody, err := buildOpenAIImagesResponsesRequest(parsed, parsed.Model, openAIImagesResponsesRequestOptions{
		MainModel:       imageSettings.mainModel,
		ReasoningEffort: imageSettings.reasoningEffort,
	})
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Failed to build image request: %s", err.Error()))
	}
	s.sendEvent(c, TestEvent{Type: "content", Text: fmt.Sprintf("Responses driver: %s; image model: %s\n", imageSettings.mainModel, parsed.Model)})
