...
func TestBuildGrokResponsesRequest_HeaderOverrideCannotInjectResponsesLite(t *testing.T) {
	account := &Account{
	...
	req, err := buildGrokResponsesRequest(
		...
		nil,
		account,
		[]byte(`{"model":"grok-4.5","input":"hello"}`),
		"access-token",
		"",
		&config.Config{},
	)
	...
	require.Equal(t, "override-applied", getHeaderRaw(req.Header, "x-custom"))
}
...
func TestForwardAsChatCompletionsForGrokStopFallsBackToXAIChatCompletions(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	body := []byte(`{"model":"grok","messages":[{"role":"user","content":"hi"}],"reasoning_effort":"xhigh","stream":false,"stop":"done","prompt_cache_key":"raw-client-cache-key"}`)
