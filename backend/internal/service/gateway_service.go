...
// DoGrokNativeResponsesJSON 向指定账号的 Grok 上游发送非流式 Responses 请求并返回原始 JSON。
// 该入口供 /v1/web_search 使用，不依赖 Gin 上下文，始终采用固定的 Grok CLI 身份。
//
// @param ctx 请求上下文。
// @param account Grok 账号。
// @param body Responses JSON 请求体。
// @return 上游原始 JSON 响应体；请求失败时返回错误。
