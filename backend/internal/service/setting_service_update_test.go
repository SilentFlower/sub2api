...
func TestSettingService_UpdateSettings_OpenAIResponsesLiteHeaderBlockedModels(t *testing.T) {
	...
	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		OpenAIResponsesLiteHeaderBlockedModels: []string{" gpt-5.4* ", "gpt-5.5", "gpt-5.4*"},
	})
	require.NoError(t, err)
	require.JSONEq(t, `["gpt-5.4*","gpt-5.5"]`, repo.updates[SettingKeyOpenAIResponsesLiteHeaderBlockedModels])

	err = svc.UpdateSettings(context.Background(), &SystemSettings{
		OpenAIResponsesLiteHeaderBlockedModels: []string{},
	})
	require.NoError(t, err)
	require.JSONEq(t, `[]`, repo.updates[SettingKeyOpenAIResponsesLiteHeaderBlockedModels])

	err = svc.UpdateSettings(context.Background(), &SystemSettings{
		OpenAIResponsesLiteHeaderBlockedModels: []string{"gpt-5.*-mini"},
	})
	require.Error(t, err)
