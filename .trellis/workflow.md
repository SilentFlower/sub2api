...
`trellis-push` owns all detailed plan/result presentation; this hub must not duplicate its templates, field order, repository labels, retained-dirty wording, or display thresholds. The hub only requires one ordinary confirmation over the exact file set and commit message. Before approval, do not `git add`, commit, or push. Ordinary commits use exact paths with `git commit --only`; never use `git add -A` / `git add .`. Unrelated dirty/staged paths stay retained and do not block an ordinary exact commit, while unknown ahead commits remain a branch-level push risk.

An already-started merge is the only merge-aware exception. `trellis-push` may finalize it in ordinary push mode only when `MERGE_HEAD` exists, `git ls-files -u` is empty, every staged path belongs to the confirmed exact planned set, and there are no retained staged paths. After approval it stages only the exact planned paths, verifies the cached path set equals the plan and `git diff --cached --check` passes, then runs one pathless `git commit -m` because Git forbids partial commits during a merge. It must verify the resulting commit has exactly the recorded pre-merge `HEAD` and `MERGE_HEAD` as parents. This exception does not allow `trellis-push` to start, abort, continue conflict resolution for, or otherwise manage a merge, and it is unavailable to user/auto-loop commit-only modes.
