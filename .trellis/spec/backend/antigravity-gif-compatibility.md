...
- 输入支持纯 base64、`data:image/gif;base64,`，以及部分客户端产生的单层 `base64:data:image/gif;base64,` 包装；解析器最多剥离一层外部 `base64:` 标记，再校验内部 data URI。base64 payload 只按标准 padded/raw base64 解码，不自动 URL 反转义、不接受 base64url，也不移除中间空白。输出必须是按时间顺序排列、无任何前缀的 `image/png` base64 part。非 GIF part、未知字段和相对顺序必须保留。
...
| 无法按标准 padded/raw base64 解码 GIF payload | 本地 400，同时日志包含 `gif_stage=base64_decode` 等安全诊断字段，便于判别是 payload 形态错误、padding 问题还是真实损坏 |
