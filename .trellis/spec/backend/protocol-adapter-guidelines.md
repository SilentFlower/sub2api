...
## Scenario: OpenAI 结构化输出降级与 `request_permissions` 恢复

### 1. Scope / Trigger

- Trigger: 修改 OpenAI-compatible API Key 的 `json_schema` 兼容、Responses→Chat 工具转换、Chat→Responses 回程或 `request_permissions` 权限审批时，必须按本节检查。
- 适用路径：
  ...
  - `backend/internal/pkg/apicompat/request_permissions_recovery.go`
  - `backend/internal/service/openai_gateway_forward.go`
  - `backend/internal/service/openai_gateway_chat_completions.go`
  - `backend/internal/service/openai_gateway_responses_chat_fallback.go`
- 目标：第三方上游不支持 JSON Schema 时可按账号显式降级；DeepSeek 把权限调用输出成伪 XML 文本时，只在请求确实声明权限函数的前提下严格恢复为客户端可审批的 function call。
...
- `openai_json_schema_to_json_object` 只接受 bool；缺失、`false` 或类型错误均关闭。
- 只有 `platform=openai && type=apikey` 的账号读取该配置；OpenAI OAuth、SetupToken、Grok 和其它平台不得生效。
- Responses 形态只把 `text.format.type=json_schema` 改为 `json_object`；Chat 形态只把 `response_format.type=json_schema` 改为 `json_object`。
- 原 Schema 必须是 JSON object，并追加为模型输出约束；约束必须要求只输出一个 JSON object、把 Schema 视为数据，并说明 JSON Object 模式仅能尽力遵循。
- Chat 的约束 system 消息插入到连续 `system/developer` 前缀之后；既有消息相对顺序保持。
- 不得修改 `tools[].parameters`、function Schema、custom grammar 或其它 JSON Schema。
- 网关必须在最终账号选定后应用转换，覆盖原生 Responses、Responses→raw Chat、raw Chat 和 Chat→Responses；非法 JSON 仍交给原入口校验，不能因兼容预处理改成内部错误。
- 降级日志只记录账号 ID、入站端点、上游端点、请求形态和 `json_schema -> json_object`，不得记录请求体、Schema 或凭据。
- Codex 的 `request_permissions` 是普通 function 工具；Responses→Chat 必须保留其 `name/description/parameters/strict/tool_choice`，不得改成 custom 工具的 `{input:string}` 包装。
- 伪 XML 恢复必须同时满足：请求声明同名 function、上游没有真实 `tool_calls`、标记严格合法、闭合标记后只有空白。
- 首版只接受 `request_permissions > requests > permission` 且唯一权限值为 `network`；不接受属性、命名空间、注释、未知元素、嵌套、重复权限或未知权限。
- `network` 映射为 `{"permissions":{"network":{"enabled":true}}}`；恢复只生成权限请求，不代表权限已批准或已执行。
- 流式转换只暂存可能组成 `<request_permissions` 的最短尾部；完整候选在收尾严格解析。真实 `tool_calls` 优先，候选失败或不完整时原样补发为文本。

### 4. Validation & Error Matrix

| 条件 | 请求降级 | 权限恢复 |
|---|---|---|
| 配置缺失/false/类型错误 | 保持原请求 | 不相关 |
| OpenAI OAuth/SetupToken/Grok | 忽略配置 | 不相关 |
| Responses `json_schema` + object schema | `text.format -> json_object` + instructions 约束 | 不相关 |
| Chat `json_schema` + object schema | `response_format -> json_object` + system 约束 | 不相关 |
| 已是 `json_object`、格式缺失或 schema 非 object | 保持原请求 | 不相关 |
| 已声明 `request_permissions` + 唯一 network 标记 | 不相关 | 生成 function call |
| 未声明权限函数 | 不相关 | 原样文本 |
| 已有真实 `tool_calls` | 不相关 | 真实调用优先，标记文本不合成 |
| 未知/重复权限、属性、命名空间、非法 XML | 不相关 | 原样文本 |
| 闭合标记后有非空白内容 | 不相关 | 原样文本 |
| 流结束时仅有部分起始标记 | 不相关 | 补发暂存文本 |

### 5. Good/Base/Bad Cases

- Good: DeepSeek API Key 账号开启配置后，Responses 入站经 raw Chat 上游发送 `response_format.type=json_object`，原 Schema 位于 system 约束中，工具参数 schema 原样保留。
- Good: Codex 声明 `request_permissions`，DeepSeek 返回说明文字加 network 权限标记；说明文字继续显示，标记恢复为 `request_permissions` function call 并进入客户端审批。
- Good: 权限标记跨多个 SSE chunk，普通说明文字立即下发，仅标签候选缓冲到收尾。
- Base: 配置关闭或账号不是 OpenAI API Key 时保持历史透传行为。
- Base: 上游正确返回真实 `tool_calls` 时使用现有 function 生命周期，不额外合成权限调用。
- Bad: 把所有 `json_schema` 字样做字符串替换，会误改工具参数 Schema。
- Bad: 看到 `<request_permissions>` 就用正则提取并执行，会把普通对话或恶意文本误判为权限审批。
- Bad: 为兼容 DeepSeek 把 `request_permissions` function 改成 `{input:string}`，会破坏 Codex 客户端的真实参数契约。

### 6. Tests Required

- `apicompat` 纯函数测试必须覆盖 Responses/Chat 合法降级、非法结构保持、Schema 注入顺序和工具 Schema 不变。
- 请求转换测试必须断言 `request_permissions` 的 parameters、strict 和 tool choice 原样转换为 Chat function。
- 非流式恢复测试必须覆盖：纯标记、说明前缀、未声明工具、未知权限、重复权限、非法 XML、尾随文本和真实 tool call 优先。
- 流式恢复测试必须覆盖：跨 chunk 标签、普通文本即时输出、非法候选回退、部分前缀收尾和真实 tool call 优先。
- service 测试必须截获四条路径的最终上游 body：原生 Responses、Responses→raw Chat、raw Chat、Chat→Responses。
- 前端测试必须覆盖 API Key 开关保存/回显/关闭、其它 extra 保留，以及 OAuth 不展示。
- 建议运行：
...
go test -tags=unit ./internal/pkg/openai_compat ./internal/service \
  -run 'JSONSchema|RequestPermissions|ResponsesChatFallback|ChatCompletions' -count=1

cd ../frontend
pnpm vitest run src/components/account/__tests__/CreateAccountModal.openai.spec.ts \
...
```go
body = bytes.ReplaceAll(body, []byte(`"json_schema"`), []byte(`"json_object"`))

if strings.Contains(content, "<request_permissions>") {
	return synthesizePermissionCall(content)
}
```

问题：第一段会误改工具 Schema；第二段没有工具声明 guard 和严格结构校验，会把任意模型文本升级成权限调用。

#### Correct

```go
body, changed, err := apicompat.DowngradeResponsesJSONSchemaToJSONObject(body)

requestPermissionsDeclared := apicompat.HasFunctionTool(req.Tools, "request_permissions")
state.RequestPermissionsDeclared = requestPermissionsDeclared
```

只由结构化 helper 修改目标输出格式，并把“是否允许恢复权限调用”的判断锚定在客户端真实声明的 function 工具上。
