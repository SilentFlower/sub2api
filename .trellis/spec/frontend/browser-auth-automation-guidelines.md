...
```json
{
  ...
  "verification_launch_url": "优先带 user_code 的可信 xAI Device Flow 浏览器启动页",
  "created_at": 0,
  "expires_at": 0,
...
| 未提交密码前进入 Device Sign-in 页 | 先等待并识别该页自己的邮箱登录入口；若没有登录控件且等待窗口耗尽，则回到 `https://accounts.x.ai/sign-in`，不填写 `user_code` |
...
- Good：xAI Device Flow 返回 `verification_uri: "https://accounts.x.ai/oauth2/device"` 与 `verification_uri_complete` 时，任务保存基础验证页和带 `user_code` 的启动页，登录标签先打开 `verification_uri_complete`；未登录时在该页选择 `Login with email`，没有登录控件时才兜底回 `accounts.x.ai/sign-in`。
...
- Bad：未提交密码前看到 `/oauth2/device` 就填写设备码，或立即强制跳走而没有给官方 Device 页自己的邮箱登录入口加载时间，导致 Device Flow 上下文丢失。
...
- Node VM 浏览器状态机测试至少覆盖：
  ...
  - 未提交密码前进入 Device Sign-in 页时，若页面已有邮箱登录入口则优先点击；若只有设备码输入框则先等待、不填写，等待窗口耗尽后才回到邮箱登录入口；密码提交后中文 Device Sign-in 页才通过输入框附近文本识别设备码输入框，并提交当前任务 `user_code`。
  - 密码填入并提交一次，提交后共享密码删除。
  - 共享任务取消后待执行动作被取消。
