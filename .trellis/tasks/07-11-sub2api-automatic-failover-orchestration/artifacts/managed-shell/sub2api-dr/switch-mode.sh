...
verify_ha_automation_context() {
  [ "${HA_AUTOMATION:-false}" = "true" ] || return 0
  ...
  local verifier="${HA_VERIFY_COMMAND:-/usr/local/libexec/sub2api-ha-verify-action}"
...
load_current_state() {
  ...
  if command -v timedatectl >/dev/null 2>&1; then
    case "$(timedatectl show -p NTPSynchronized --value 2>/dev/null || true)" in
      yes) ntp_synchronized=yes ;;
      no) ntp_synchronized=no ;;
      *) ntp_synchronized=unknown ;;
    esac
  fi
  ...
  if [ "${postgres_container_state}" = "running" ]; then
    ...
    if [ "${postgres_recovery}" = "t" ]; then
      postgres_streaming="$(docker exec -e PGPASSWORD="${POSTGRES_PASSWORD}" "${POSTGRES_CONTAINER}" \
        psql -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -Atc \
        "SELECT COALESCE((SELECT status FROM pg_stat_wal_receiver LIMIT 1), 'none');")"
...
print_current_state() {
  ...
  printf '模式：%s\n' "${current_mode}"
  printf 'PostgreSQL：container=%s recovery=%s streaming=%s\n' \
    "${postgres_container_state}" "${postgres_recovery}" "${postgres_streaming}"
...
ensure_standby() {
  ...
  [ "${postgres_streaming}" = "streaming" ] || die "B PostgreSQL WAL receiver不是 streaming"
  [ "${ntp_synchronized}" = "yes" ] || die "B NTP 未同步，拒绝进入 standby 门禁"
