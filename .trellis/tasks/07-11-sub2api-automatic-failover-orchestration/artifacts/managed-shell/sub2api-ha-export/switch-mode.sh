...
用法：
  ...
  switch-mode.sh prepare-handoff [--dry-run]
  switch-mode.sh promote-after-handoff [--dry-run]
...
说明：
  ...
  prepare-handoff    B 持有租约时冻结 B 写入并等待 A 完全追平，但不提升 A。
  promote-after-handoff
                     Durable Object 已把租约交给 A 后，提升 A 并启动应用。
...
confirm_action() {
  local token="$1"
  local prompt="$2"
  local confirmation verifier

  if [ "${HA_AUTOMATION:-false}" = "true" ]; then
    [[ "${HA_EPOCH:-}" =~ ^[0-9]+$ ]] || die "自动化调用缺少有效 HA_EPOCH"
    [ -n "${HA_TRANSITION_ID:-}" ] || die "自动化调用缺少 HA_TRANSITION_ID"
    [ -n "${HA_EXPECTED_STATE:-}" ] || die "自动化调用缺少 HA_EXPECTED_STATE"
    [ -n "${HA_EXPECTED_OWNER:-}" ] || die "自动化调用缺少 HA_EXPECTED_OWNER"
    verifier="${HA_VERIFY_COMMAND:-/usr/local/libexec/sub2api-ha-verify-action}"
    [ -x "${verifier}" ] || die "自动化租约验证器不可执行：${verifier}"
    "${verifier}" \
      --config "${HA_AGENT_CONFIG:-/etc/sub2api-ha/agent.json}" \
      --expected-epoch "${HA_EPOCH}" \
      --owner "${HA_EXPECTED_OWNER}" \
      --expected-state "${HA_EXPECTED_STATE}" \
      --transition-id "${HA_TRANSITION_ID}" >/dev/null
    return 0
  fi

  read -r -p "${prompt}" confirmation
  [ "${confirmation}" = "${token}" ] || die "确认口令不匹配，已取消"
}
...
prepare_from_b() {
  local dry_run="$1"
  local local_output remote_output local_mode b_mode
  ...
  confirm_action STOP_AND_REBUILD_A_FROM_B '请输入 STOP_AND_REBUILD_A_FROM_B 继续：'
...
prepare_handoff() {
  local dry_run="$1"
  local local_output remote_output local_mode b_mode target_lsn target_redis_offset

  local_output="$(local_state)"
  local_mode="$(field_value "${local_output}" mode)"
  [ "${local_mode}" = "standby-from-b" ] \
    || die "A 当前模式为 ${local_mode}，只有 B 的追平备节点可以准备 handoff"

  remote_output="$(b_state)"
  b_mode="$(field_value "${remote_output}" mode)"
  case "${b_mode}" in
    active|active-stopped) ;;
    *) die "B 当前模式为 ${b_mode}，不能准备 handoff" ;;
  esac
  verify_recovery_image_matches_b "${remote_output}"

  if [ "${dry_run}" = "true" ]; then
    ssh_b "cd '${B_DR_ROOT}' && ./scripts/switch-mode.sh freeze --dry-run"
    printf 'dry-run：将冻结 B 应用并等待 A 达到 B 的 PostgreSQL LSN和 Redis offset，但不会提升 A。\n'
    printf 'dry-run 完成：没有停止 B 应用、提升 A 数据库或启动 A 应用。\n'
    return 0
  fi

  printf '%s\n' '警告：该阶段只冻结 B 写入并等待 A 追平；完成后必须先提交 Durable Object handoff。'
  confirm_action FREEZE_B_WRITES_FOR_HANDOFF '请输入 FREEZE_B_WRITES_FOR_HANDOFF 继续：'

  ssh_b "cd '${B_DR_ROOT}' && printf '%s\\n' FREEZE_B_WRITES_FOR_CUTBACK | ./scripts/switch-mode.sh freeze"
  remote_output="$(b_state)"
  ...
  write_state a-handoff-ready
  printf 'A 已达到 B 冻结点，尚未提升数据库；现在可以提交 Durable Object handoff。\n'
}
...
promote_after_handoff() {
  ...
  case "${local_mode}" in
    standby-from-b|cutback-postgres-promoted|active-recovered-stopped) ;;
    *) die "A 当前模式为 ${local_mode}，不允许在 handoff 后提升" ;;
  esac

  remote_output="$(b_state)"
  b_mode="$(field_value "${remote_output}" mode)"
  [ "${b_mode}" = "active-stopped" ] \
    || die "B 当前模式为 ${b_mode}，handoff 后提升前必须保持 active-stopped"
  verify_recovery_image_matches_b "${remote_output}"

  if [ "${dry_run}" = "true" ]; then
    printf 'dry-run：将复核 A 仍达到 B 冻结点，再提升 PostgreSQL、Redis并启动 A 应用。\n'
    printf 'dry-run 完成：没有提升数据库或启动 A 应用。\n'
    return 0
  fi

  printf '%s\n' '警告：只有 Durable Object 已把新 epoch 租约交给 A 时才能继续。'
  confirm_action PROMOTE_A_AFTER_LEASE_HANDOFF '请输入 PROMOTE_A_AFTER_LEASE_HANDOFF 继续：'
  ...
  printf 'A 已在新 epoch 下提升并启动，等待自动入口切换和 B 重新入备。\n'
}
...
restore_b_standby() {
  ...
  if [ "${b_mode}" = "standby" ]; then
    ...
    confirm_action SYNC_A_APP_DATA_TO_B '请输入 SYNC_A_APP_DATA_TO_B 继续：'
  ...
  confirm_action A_IS_PRIMARY_REBUILD_B_STANDBY '请输入 A_IS_PRIMARY_REBUILD_B_STANDBY 继续：'
...
main() {
  ...
  case "${action}:${option}" in
    ...
    sync-release:|prepare-from-b:|prepare-handoff:|promote-after-handoff:|cutback-to-a:|restore-b-standby:) ;;
    sync-release:--dry-run|prepare-from-b:--dry-run|prepare-handoff:--dry-run|promote-after-handoff:--dry-run|cutback-to-a:--dry-run|restore-b-standby:--dry-run) dry_run=true ;;
  ...
  case "${action}" in
    ...
    prepare-handoff) prepare_handoff "${dry_run}" ;;
    promote-after-handoff) promote_after_handoff "${dry_run}" ;;
