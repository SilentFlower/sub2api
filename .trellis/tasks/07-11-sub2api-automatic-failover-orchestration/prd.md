...
- 租约 TTL 固定为 30 秒，节点每 5 秒续租或观察；`epoch` 只能单调递增。
...
- 当前写入节点连续无法联系 Durable Object 且 30 秒租约到期时，必须停止 Sub2API 应用；数据库现场可以保留，但不得继续对外写入。
...
- 两节点每 5 秒一次控制面请求的理论基线约为 34,560 次/天，重试和附加检查必须有上限。
...
- [ ] Durable Object 不可达超过 30 秒时，当前应用自动停止且 B 不提升；恢复后不会出现两个节点同时恢复写入。
