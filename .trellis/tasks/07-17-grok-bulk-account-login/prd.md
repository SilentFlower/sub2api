...
- `www.havefun.eu.cc` 当前没有正确配置到 Sub2API nginx 虚拟主机，TLS 证书主机名也不匹配；脚本可以开发，但真实使用前浏览器必须能正常打开该页面。
...
- 只在 `https://www.havefun.eu.cc/*` 渲染控制台；即使用户手工扩大用户脚本匹配范围，运行时也必须拒绝 HTTP 页面。
...
- 脚本使用返回的 `verification_uri_complete` 打开登录标签，并按服务端返回的 `interval` 轮询 `https://auth.x.ai/oauth2/token`。
...
- 脚本发现运行器不是 Violentmonkey、权限缺失、站点域名不可访问或版本能力不足时，必须阻止启动并显示明确提示。
- 不修改 Sub2API 后端、前端、数据库或现网配置。

## 验收标准

- [ ] 合法行、空行、非法行、重复邮箱和密码含额外 `|` 的输入都能得到正确解析结果。
- [ ] 控制台只在 `www.havefun.eu.cc` 显示，xAI/Grok 页面不会出现脚本面板或明文凭据。
