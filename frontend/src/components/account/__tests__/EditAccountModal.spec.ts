...
describe('EditAccountModal', () => {
  ...
  it('同时保存 JSON Schema 与生图回填开关并保留既有 extra', async () => {
    const account = buildAccount()
    account.extra = {
      openai_responses_supported: false,
...
describe('EditAccountModal Alpha Search 经上游 Responses 开关', () => {
  beforeEach(() => {
    updateAccountMock.mockReset().mockResolvedValue(buildAccount())
    checkMixedChannelRiskMock.mockReset().mockResolvedValue({ has_risk: false })
  })
  ...
  it('关闭开关时删除 extra 键并保留其它键', async () => {
    const account = buildAccount()
    account.extra = { openai_alpha_search_via_responses: true, keep_me: 1 } as Record<string, unknown>
    const wrapper = mountModal(account)
    const toggle = wrapper.get('[data-testid="alpha-search-via-responses-toggle"]')
    expect(toggle.attributes('aria-checked')).toBe('true')
    ...
    const extra = updateAccountMock.mock.calls[0]?.[1]?.extra as Record<string, unknown>
    expect(extra).not.toHaveProperty('openai_alpha_search_via_responses')
    expect(extra.keep_me).toBe(1)
  })

  it('DeepSeek API Key 与 OpenAI OAuth 账号都不显示开关', () => {
    const deepseek = buildAccount()
    deepseek.platform = 'deepseek'
    deepseek.credentials = {
      api_key: 'sk-ds',
    ...
    expect(mountModal(deepseek).find('[data-testid="alpha-search-via-responses-toggle"]').exists()).toBe(false)

    const oauth = buildAccount()
    oauth.type = 'oauth'
    oauth.credentials = { access_token: 'token' } as unknown as typeof oauth.credentials
    expect(mountModal(oauth).find('[data-testid="alpha-search-via-responses-toggle"]').exists()).toBe(false)
  })
})
