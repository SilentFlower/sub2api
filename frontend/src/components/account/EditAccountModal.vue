...
function loadQuotaControlSettings(account: Account) {
  ...
  if (account.anthropic_cch_enabled === true) {
    anthropicCCHEnabled.value = true
    anthropicCCHMode.value = account.anthropic_cch_mode === 'user_agent' ? 'user_agent' : 'fixed'
    anthropicCCHFixedVersion.value = account.anthropic_cch_fixed_version || '2.1.90'
    anthropicCCHRewriteUserAgent.value = account.anthropic_cch_rewrite_user_agent !== false
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'anthropic' && (props.account.type === 'oauth' || props.account.type === 'setup-token')) {
      ...
      // Anthropic CCH override setting
      if (anthropicCCHEnabled.value) {
        newExtra.anthropic_cch_enabled = true
        newExtra.anthropic_cch_mode = anthropicCCHMode.value
        if (anthropicCCHFixedVersion.value.trim()) {
          newExtra.anthropic_cch_fixed_version = anthropicCCHFixedVersion.value.trim()
        } else {
          delete newExtra.anthropic_cch_fixed_version
        }
        if (anthropicCCHMode.value === 'fixed') {
          newExtra.anthropic_cch_rewrite_user_agent = anthropicCCHRewriteUserAgent.value
        } else {
          delete newExtra.anthropic_cch_rewrite_user_agent
        }
      } else {
        delete newExtra.anthropic_cch_enabled
        delete newExtra.anthropic_cch_mode
        delete newExtra.anthropic_cch_fixed_version
        delete newExtra.anthropic_cch_rewrite_user_agent
      }
